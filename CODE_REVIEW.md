...
- **`chatServer_msgpack`의 오래된 간접 의존성(`golang.org/x/net` 등)을 정리했다.** `github.com/vmihailenco/msgpack/v4`에 `appengine.go`라는 파일이 있는데, `// +build appengine` 빌드 태그가 붙어 있어 우리가 평소 빌드할 때는 전혀 컴파일되지 않는다. 하지만 이 파일이 `google.golang.org/appengine/datastore`를 import하고 있어서, Go 모듈 시스템은 "혹시 나중에 `-tags appengine`으로 빌드할 수도 있으니" 완전성을 위해 `google.golang.org/appengine`과 그 하위 의존성(`golang.org/x/net`, `golang.org/x/crypto` 등)을 `go.sum`에 계속 포함시킨다. 실제로는 한 번도 컴파일/링크되지 않는 "유령 의존성"이지만, GitHub Dependabot은 도달 가능성과 무관하게 `go.sum`에 있는 버전만 보고 취약점을 신고하므로 실제 문제가 된다.
    - 처음에는 `go get -u golang.org/x/net golang.org/x/text google.golang.org/appengine github.com/golang/protobuf && go mod tidy`로 몇 개만 골라서 올렸는데, 이게 오히려 appengine이 요구하는 또 다른 오래된 전이 의존성들(`golang.org/x/crypto`, `x/tools`, `x/mod`, `x/sync`, `x/sys`, `x/term`, `x/xerrors` 등, MVS 규칙상 "필요한 최소 버전"만 선택되기 때문)을 새로 끌어들여서 Dependabot 경고가 1건에서 15건(높음 7 + 중간 8)으로 오히려 늘어났다. `go mod tidy`는 실제로 import되지 않는(도달 불가능한) 모듈에 대한 `go get`의 수동 버전 지정을 조용히 무시하고 되돌리기 때문에, 단순히 개별 패키지를 `go get -u`하는 것만으로는 이 유령 의존성들을 최신화할 수 없었다.
    - 최종적으로는 `go.mod`에 `replace` 블록을 추가해 `golang.org/x/net`, `x/crypto`, `x/mod`, `x/sync`, `x/sys`, `x/term`, `x/text`, `x/tools`, `x/xerrors`를 각각 알려진 취약점이 없는 최신 버전으로 강제 고정했다(`replace`는 `require`와 달리 도달 불가능한 모듈에 대해서도 `go mod tidy`가 무시하지 않는다). `github.com/vmihailenco/msgpack/v4`도 v4.2.1 → v4.3.13으로 올렸다. 이 과정에서 `go.mod`의 Go 버전 요구사항이 `go 1.25.0`으로 자동 상향되었다(최신 `golang.org/x/net`이 요구하는 최소 버전). 다른 5개 모듈은 영향받지 않고 `go 1.21`을 유지한다.
    - `govulncheck -show verbose`로 6개 모듈을 전수 검사해, 남은 항목이 전부 "Standard library"(로컬 Go 툴체인 버전에 딸린 것으로, 이 저장소의 의존성 선택과 무관함)뿐이고 서드파티 패키지발 취약점은 더 이상 없음을 확인했다.
