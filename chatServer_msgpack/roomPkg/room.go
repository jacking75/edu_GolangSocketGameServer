...
func (room *baseRoom) settingPacketFunction() {
	...
	room._funcPackeIdlist = make([]uint16, 0, maxFuncListCount)
...
func (room *baseRoom) _addPacketFunction(packetID uint16,
...
// 함수 이름 바꾸는 것이 좋을 듯
func (room *baseRoom) allocAllUserInfo(exceptSessionUniqueId uint64) (userCount int16, uniqueIdList []uint64, idList []string) {
	...
	uniqueIdList = make([]uint64, 0, userCount)
	idList = make([]string, 0, userCount)
	...
	for _, user := range room._userSessionUniqueIdMap {
		...
		uniqueIdList = append(uniqueIdList, user.RoomUniqueId)
		idList = append(idList, string(user.ID[0:user.IDLen]))
	}

	return userCount, uniqueIdList, idList
}
...
func (room *baseRoom) broadcastPacket(packetSize uint16,
	sendPacket []byte,
