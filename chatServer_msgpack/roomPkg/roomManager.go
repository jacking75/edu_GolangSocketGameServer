...
func (roomMgr *RoomManager) PacketProcess(roomNumber int32, packet protocol.Packet) {
	NTELIB_LOG_DEBUG("[[RoomManager - PacketProcess]]", zap.Uint16("PacketID", packet.Id))
	...
	if roomNumber == -1 && packet.Id == protocol.PACKET_ID_ROOM_ENTER_REQ {
		...
		roomNumber = int32(requestPacket.RoomNumber)
	}
	...
	for i := 0; i < funcCount; i++ {
		...
		if result != protocol.ERROR_CODE_NONE {
			NTELIB_LOG_DEBUG("[[Room - _packetProcess - Fail]]",
				zap.Uint16("PacketID", packet.Id), zap.Int16("Error", result))
		}

		return
	}

	NTELIB_LOG_DEBUG("[[Room - _packetProcess - Fail(Not Registered)]]", zap.Uint16("PacketID", packet.Id))
}
