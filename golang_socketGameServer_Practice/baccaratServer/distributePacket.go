...
func (server *ChatServer) PacketProcess_goroutine_Impl() bool {
	...
	secondTimeticker := time.NewTicker(time.Millisecond * time.Duration(server.RoomMgr.RoomStateCheckPeriod))
	defer secondTimeticker.Stop()
	checkRoomStateChecker := 0
	...
	for {
		...
		case curTime := <-secondTimeticker.C:
			{
				if server.RoomMgr.CheckRoomState(curTime.Unix(), checkRoomStateChecker) == true {
					checkRoomStateChecker = 0
				} else {
					checkRoomStateChecker += server.RoomMgr.RoomStateCheckNumber
				}
			}
		}
	}
...
func ProcessPacketLogin(sessionIndex int32,
	...
	PassWD := bytes.Trim(request.PassWD[:], "\x00");
