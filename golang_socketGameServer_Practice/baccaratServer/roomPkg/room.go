...
type baseRoom struct {
	...
	_roomUserList []*roomUser
	_roomUserListCache []*roomUser
...
func (room *baseRoom) changeState(state int) {
	...
	} else if room._curState == ROOM_STATE_GAME_RESULT {
		room.clearRoomUserListCache()
	}

	fmt.Println("[now state] ",state,", [_statusChangeCompletionMillSec] ",room._gameLogic._statusChangeCompletionMillSec,", [NetLib_GetCurrnetUnixTime()] ",NetLib_GetCurrnetUnixTime())
}
...
func (room *baseRoom) _settingMasterUser() {
	...
	room.changeMasterUser(masterUniqueId)
}
...
func (room *baseRoom) _initialize(index int32, config RoomConfig) {
	...
	room._masterUserSessionUniqueId = 0
	room._roomUserList = make([]*roomUser, config.MaxUserCount, config.MaxUserCount)
	room._roomUserListCache = make([]*roomUser, config.MaxUserCount, config.MaxUserCount)
}

func (room *baseRoom) clearRoomUserListCache() {
	room._roomUserListCache = room._roomUserListCache[:0]
}
...
func (room *baseRoom) settingPacketFunction() {
	...
	room._addPacketFunction(protocol.PACKET_ID_ROOM_WHISPER_REQ, room._packetProcess_Whisper)
}
...
func (room *baseRoom) addUser(userInfo addRoomUserInfo) (*roomUser, int16) {
	...
	if room.getCurUserCount() == 1 {
		room.changeMasterUser(userInfo.netSessionUniqueId)
	}

	return user, protocol.ERROR_CODE_NONE
}

func (room *baseRoom) changeMasterUser(userSessionUniqueId uint64) {
	room._masterUserSessionUniqueId = userSessionUniqueId

	notify := protocol.RoomMasterChangeNtfPacket{ room._masterUserSessionUniqueId }
...
func (room *baseRoom) secondTimeEvent() {
	//TODO 주기적으로 방의 유저가 연결 되어 있는지 확인 필요 (impl)
	for _, user := range room._userSessionUniqueIdMap {
		if NetLibIIsAliveSession(user.netSessionIndex, user.netSessionUniqueId) == false {
			fmt.Println(user.netSessionUniqueId)
		}
	}
}
...
func (room *baseRoom) whisperPacket(SendUserUniqueId uint64, ReceiveUserUniqueId uint64, packetSize int16, sendPacket []byte) int16 {
	receiveUser := room._userSessionUniqueIdMap[ReceiveUserUniqueId]

	if receiveUser == nil {
		return protocol.ERROR_CODE_USER_NOT_IN_ROOM
	} else {
		sendUser := room._userSessionUniqueIdMap[SendUserUniqueId]
		NetLibIPostSendToClient(sendUser.netSessionIndex, sendUser.netSessionUniqueId, sendPacket)
		NetLibIPostSendToClient(receiveUser.netSessionIndex, receiveUser.netSessionUniqueId, sendPacket)
	}

	return protocol.ERROR_CODE_NONE
}

func (room *baseRoom) deliveryPacket(UserUniqueId uint64, packetSize int16, sendPacket []byte) {
	receiveUser := room._userSessionUniqueIdMap[UserUniqueId]
...
func (room *baseRoom) endGame() {
	for _, user := range room._userSessionUniqueIdMap {
		if user.isBattingDone() == false {
			user.selectBat = BATTING_SELECT_PLAYER
			_sendRoomGameBattingNotify(room, user.RoomUniqueId, BATTING_SELECT_PLAYER)
		}
	}
	...
	notify.StatusChangeCompletionMillSec = room._gameLogic._statusChangeCompletionMillSec
	...
	for _, user := range room._userSessionUniqueIdMap {
		...
		if user == nil {
			break
		} else if user.selectBat == gameResult.result && gameResult.result == BATTING_SELECT_PLAYER {
			sql = "Dollar = Dollar+20, Win = Win+1"
		} else if user.selectBat == gameResult.result && gameResult.result == BATTING_SELECT_BANKER {
			sql = "Dollar = Dollar+17, Win = Win+1"
		} else if user.selectBat == gameResult.result && gameResult.result == BATTING_SELECT_TIE {
			sql = "Dollar = Dollar+90, Win = Win+1"
		} else {
			sql = "Dollar = Dollar-10, Lose = Lose+1"
		}
		userID := bytes.Trim(user.ID[:], "\x00")
		protocol.Update(sql, userID)

		Dollar, Plays, Win, Lose := protocol.Select(userID)
		userInfoNotify := protocol.RoomUserInfoNtfPacket{Dollar, Plays, Win, Lose}
		notifySendBuf, packetSize := userInfoNotify.EncodingPacket()
		room.deliveryPacket(user.netSessionUniqueId, packetSize, notifySendBuf)
	}

	for _, user := range room._roomUserListCache {
		sql := ""
		if user == nil {
			break
		} else if user.selectBat == gameResult.result && gameResult.result == BATTING_SELECT_PLAYER {
