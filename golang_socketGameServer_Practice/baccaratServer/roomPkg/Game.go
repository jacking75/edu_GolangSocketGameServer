...
func (game *baccaratGame) setBattingWaitTime(curMilliSec int64) {
	game._statusChangeCompletionMillSec = curMilliSec + BATTING_WAIT_SEC
}
...
// 게임 종료
func (game *baccaratGame) _End(endResult *baccaratGameResultInfo) {
	...
	game._statusChangeCompletionMillSec = NetLib_GetCurrnetUnixTime() + NEXT_GAME_WAIT_SEC
}
...
func _baccaratCardIndexToScore(cardIndex int8) int8 {
	score := cardIndex % CARD_ROW_COUNT + 1
