...
type RoomManager struct {
	...
	_roomList      []baseRoom
	RoomStateCheckPeriod int
	RoomStateCheckNumber int
}
...
func (roomMgr *RoomManager) _initialize(config RoomConfig) {
	...
	roomMgr.RoomStateCheckNumber = config.RoomStateCheckNumber
	roomMgr.RoomStateCheckPeriod = config.RoomStateCheckPeriod
...
func (roomMgr *RoomManager) PacketProcess(roomNumber int32, packet protocol.Packet) {
	...
	fmt.Println("[",room._number," room master] ", room._masterUserSessionUniqueId, ", [curState] ", room._curState)
...
func (roomMgr *RoomManager) CheckRoomState(curTimeMilliSec int64, checkRoomStateChecker int) bool {
	//TODO 한번에 모든 방을 다 조사할 필요가 없다. 밀리세컨드 단위로 타이머를 돌게 하고 그룹 단위로 방을 조사한다
	isAllRoomCheckDone := false

	limit := checkRoomStateChecker + roomMgr.RoomStateCheckNumber
	if limit >= (int)(roomMgr._maxRoomCount) {
		limit = (int)(roomMgr._maxRoomCount)
		isAllRoomCheckDone = true
	}

	for i := checkRoomStateChecker; i < limit; i++ {
		roomMgr._roomList[i].checkState(curTimeMilliSec)
	}

	//fmt.Println("i:=",checkRoomStateChecker, "; i<",limit)

	return isAllRoomCheckDone
}
