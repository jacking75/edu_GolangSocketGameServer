...
func (room *baseRoom) _leaveUserProcess(user *roomUser) {
	...
	//TODO 방의 상태가 ROOM_STATE_NOE, ROOM_STATE_GAME_RESULT 일 때만 나갈 수 있다. (o)
	// 클라이언트 GUI 단에서 leave를 못하도록 막음

	//TODO 유저가 접속이 끊어져서 나가는 경우라면 게임이 끝날 때까지 유저 정보 들고 있다가 (o)
	//  ROOM_STATE_GAME_RESULT 상태일 때 제거한다.
	if room.isStateGameBattingWait() {
		if user.isBattingDone() {
			room._roomUserListCache = append(room._roomUserListCache, user)
		} else {
