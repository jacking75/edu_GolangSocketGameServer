...
func (user *roomUser) SetNetworkInfo(sessionIndex int32, sessionUniqueId uint64) {
	...
	user.RoomUniqueId = user.netSessionUniqueId
}
...
func (user *roomUser) initBatting() {
	user.selectBat = BATTING_SELECT_NONE
}

func (user *roomUser) isBattingDone() bool {
	return user.selectBat != BATTING_SELECT_NONE
}

func (user *roomUser) print() {
	fmt.Println("[leaveUser ",user.netSessionUniqueId,"] batting to ", user.selectBat)
}