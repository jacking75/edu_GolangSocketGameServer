...
namespace csharp_test_client
{
    public partial class mainForm : Form
    {
        ...
        {
            for (int i = 0; i <= listBoxRoomUserList.Items.Count; i++)
            {
                var item = listBoxRoomUserList.Items[i].ToString().Split(":");
                if (item[0].ToInt64() == userUniqueId)
                {
                    listBoxRoomUserList.Items.RemoveAt(i);
                    return;
                }
            }
        }
        ...
        private void btnRoomWhisper_Click(object sender, EventArgs e)
        {
            if (textBoxRoomSendWhisperMsg.Text.IsEmpty())
            {
                MessageBox.Show("채팅 메시지를 입력하세요");
                return;
            }

            var requestPkt = new RoomWhisperReqPacket();
            requestPkt.SetValue(textBoxRoomSendWhisperRecieverId.Text.ToInt64(), textBoxRoomSendWhisperMsg.Text);

            PostSendPacket(PACKET_ID.PACKET_ID_ROOM_WHISPER_REQ, requestPkt.ToBytes());
            DevLog.Write($"귓속말 요청 : {textBoxRoomSendWhisperRecieverId.Text.ToInt64()} 에게");
        }

        //게임 시작
        private void btnRoomGameStart_Click(object sender, EventArgs e)
        {
            var requestPkt = new GameStartReqPacket();

            PostSendPacket(PACKET_ID.PACKET_ID_GAME_START_REQ, new byte[]{0});
            DevLog.Write("게임 시작 요청");
        }
        
        //게임 베팅
        private void btnRoomBattingBanker_Click(object sender, EventArgs e)
        {
            var requestPkt = new GameBattingReqPacket();
            requestPkt.SetValue(2);

            PostSendPacket(PACKET_ID.PACKET_ID_GAME_BATTING_REQ, requestPkt.ToBytes());
            DevLog.Write("게임 베팅 요청 : Banker에게 베팅");
        }
        
        private void btnRoomBattingPlayer_Click(object sender, EventArgs e)
        {
            var requestPkt = new GameBattingReqPacket();
            requestPkt.SetValue(1);

            PostSendPacket(PACKET_ID.PACKET_ID_GAME_BATTING_REQ, requestPkt.ToBytes());
            DevLog.Write("게임 베팅 요청 : Player에게 베팅");
        }
        
        private void btnRoomBattingTie_Click(object sender, EventArgs e)
        {
            var requestPkt = new GameBattingReqPacket();
            requestPkt.SetValue(3);

            PostSendPacket(PACKET_ID.PACKET_ID_GAME_BATTING_REQ, requestPkt.ToBytes());
            DevLog.Write("게임 베팅 요청 : Tie에 베팅");
        }

        private void timer1_Tick_1(object sender, EventArgs e)
        {
            digitalTime.Text = DateTime.Now.ToString("mm분 ss초");
        }
    }
}
