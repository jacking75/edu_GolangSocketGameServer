...
type baseRoom struct {
	...
	_internalFunclist        []func(protocol.InternalPacket) int16
	_internalFuncPackeIdlist []int16

	//TODO 자료구조를 배열로 바꾸는 것이 좋음
...
func (room *baseRoom) _internalPacketProcess(packet protocol.InternalPacket) int16 {
	isInternal := false

	funcCount := len(room._internalFuncPackeIdlist)
	for i := 0; i < funcCount; i++ {
		if room._internalFuncPackeIdlist[i] == packet.Id {
			room._internalFunclist[i](packet)
			isInternal = true
			break
		}
	}
	...
	if isInternal{
		...
		NTELIB_LOG_ERROR("Invalid Internal Packet", zap.Int16("InternalPacketId", packet.Id))
		return protocol.ERROR_CODE_ROOM_NOT_REGISTED_INTERNAL_PACKET_ID
	}
}
...
func (room *baseRoom) settingPacketFunction() {
	...
	maxInternalFuncListCount := 8
	room._internalFunclist = make([]func(packet protocol.InternalPacket) int16, 0, maxInternalFuncListCount)
	room._internalFuncPackeIdlist = make([]int16, 0, maxInternalFuncListCount)

	room._addInternalPacketFunction(protocol.INTERNAL_PACKET_ID_DISCONNECTED_USER_TO_ROOM, room.disConnectedUser)
}
...
func (room *baseRoom) _addInternalPacketFunction(packetID int16,
	packetFunc func(protocol.InternalPacket,
		) int16) {
	room._internalFunclist = append(room._internalFunclist, packetFunc)
	room._internalFuncPackeIdlist = append(room._internalFuncPackeIdlist, packetID)
}
...
func (room *baseRoom) disConnectedUser(packet protocol.InternalPacket) int16 {
	request := protocol.InternalPacketDisConnectedUserToRoom{}
	(&request).Decoding(packet.Data)

	if room._disConnectedUser(request.SessionUniqueId) == false {
